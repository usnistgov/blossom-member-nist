...
class Environment:
    ...
    def log_environment() -> str:
        ...
        """
        message = [
            "Environment Summary: - EC2 region:\n\t{Environment.OUR_REGION}",
            " - EC2 instance:\n\t{Environment.EC2_ID}",
            " - EC2 instance:\n\t{Environment.EC2_ID}",
            " - EC2 instance:\n\t{Environment.EC2_ID}",
        ]
...
def run_ec2_commands(command: str):
    ...
    while tries < 1000:
        tries = tries + 1
        try:
            time.sleep(0.059)  # some delay always required...
...
def lambda_handler(event, context):
    ...
    """
    print(f"{event=}")  # f"event=\n{json.dumps(event, indent=2)}"
    print(f"{context=}")  # context is non-serializable
    try:
        ...
        event_types = [
            ...
            if isinstance(key, str) and key.lower() == Key.ActionType.value.lower()
                ...
                f"The {key} object is not of type str, but {type(key)} and {Key.ActionType.value=}"
            )
