...
debug "In $(basename ${BASH_SOURCE[0]}) script in $( dirname ${BASH_SOURCE[0]} ) dir"

SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd)"
source "${SCRIPT_DIR}/aws_okta_util.sh"
debug "  1: Verifying that Python/Python3 is installed in PATH"
...
debug "  2: Verifying that AWS-CLI is installed and is in PATH"
...
debug "  3: Verifying that OKTA-AWS-CLI is installed and is in PATH"
...
debug "  4: Checking if there is a currently active logged-in user"
if aws sts get-caller-identity --profile=$AWS_PROFILE > /dev/null 2>&1; then
    # Credentials are valid, no need to continue
    exit 0
fi

debug "  5: Setting everything for the python virtual environment"
# Reuse function from ./util/util-common.bash
test_setup_venv

debug "  6: Show environment context"
show_env_info "File: $0 at Line: $LINENO"
debug "$SCRIPT_DIR=${SCRIPT_DIR}"
# Run shell subprocess
debug "  7: Activate virtual environment and run login "
(
  cd "$SCRIPT_DIR"
